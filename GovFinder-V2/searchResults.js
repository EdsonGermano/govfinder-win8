...
(function () {
    ...
    ui.Pages.define(searchPageURI, {
        ...
        ready: function (element, options) {
            var resultlist = element.querySelector(".resultslist");
            if (resultlist) {
        ...
        _applyFilter: function (filter, originalResults) {
            if (filter.results === null) {
                if (filter.text === "All") {
                    filter.results = originalResults.createFiltered(function (item) { return true; });
                }
                else {
                    filter.results = originalResults.createFiltered(function (item) {
                        if (item.city != null && item.city.toLowerCase() === filter.text.toLowerCase()) {
                            return true;
                        }
                        return false;
                    });
        ...
        _generateFilters: function (originalResults) {
            this._filters = [];
            this._filters.push({ results: null, text: "All" });
            ...
            if (cityList.length > 0) {
                ...
                for (var i = 0; i < ret.length; i++) {
                    this._filters.push({ results: null, text: ret[i].replace(/\w\S*/g, function (txt) { return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase(); }) });
                }
            }      
        },
        ...
        _handleQuery: function (element, args) {
            ...
            this._generateFilters(originalResults);
        ...
        _initializeLayout: function (listView, viewState) {
            ...
            if (viewState === appViewState.snapped) {
                ...
                document.querySelector(".titlearea .pagetitle").textContent = "Gov Finder";
